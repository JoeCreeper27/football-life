...
import { GROUP_ABIL, CLUBS, CONF, DPOS, POS_WEIGHT, LV, TIER } from './data.js';
 ...
 */
export function createState(seed, { name, number, group }) {
  const rng = new Rng(seed);
  const keys = GROUP_ABIL[group];

  const ab = {};
  keys.forEach(k => (ab[k] = rng.int(20, 32)));
  ...
  const school = rng.pick(CLUBS.HS);
...
export function abCost(p, k) {
  ...
  let c = gk ? (cur >= 75 ? 4 : cur >= 70 ? 3 : cur >= 60 ? 2 : 1)
             : (cur >= 75 ? 3 : cur >= 70 ? 2 : 1);
