...
export const CLUBS = {
  ...
  /* T7 五大聯賽 */
  ENG: [
    { n: '泰晤士紅軍', t: 1 }, { n: '曼徹斯特雨城', t: 1 }, { n: '倫敦北砲', t: 1 },
    { n: '默西河藍調', t: 2 }, { n: '南岸櫻桃紅', t: 3 }, { n: '倫敦西區藍', t: 2 },
...
/**
 * 短板：這項能力低於自己「計分能力」的平均。
 * 訓練型事件卡全部掛在這個條件上 —— 加練哪一項應該是教練看著你的弱點決定的。
 * 無條件的訓練卡曾經佔掉全部抽卡的四分之一，
 * 一局生涯抽 88 張卡裡有 20 幾張是「衝刺訓練」「健身房增肌」輪流出現。
 */
const weak = (k, margin = 2) => s => {
  const p = s.player;
  if (!(k in p.ab)) return false;
  const w = POS_WEIGHT[p.dpos];
  if (!w) return true;                       // 還沒登錄位置（養成期）一律可抽
  const keys = Object.keys(w).filter(x => x in p.ab);
  const avg = keys.reduce((a, x) => a + p.ab[x], 0) / (keys.length || 1);
  return p.ab[k] < avg - margin;
};

/** 這一季真的有在踢 —— 坐看台的人不該抽到「世界波遠射」 */
const played = (m = 0.35) => s => s.club.minutes >= m;

/** 剛到一支新球隊的前幾年 */
const newAtClub = (n = 1) => s => s.club.yearsAtClub <= n;

export const EVENTS = [
  { n: '加練定位球', for: 'OUT', cond: weak('fin'), gt: '弧線繞過人牆，門將動都沒動', bt: '越練越沒手感，教練搖頭', g: { fin: 2 }, b: { fin: -2 } },
  { n: '健身房增肌', for: '*', cond: weak('phy'), gt: '對抗中站得住了', bt: '練過頭，身體變笨重', g: { phy: 2 }, b: { pac: -1, sta: -1 } },
  { n: '衝刺訓練', for: 'OUT', cond: weak('pac'), gt: '30 公尺測速跑出生涯最佳', bt: '大腿後側拉傷', g: { pac: 2 }, b: { pac: -1, inj: 10 } },
  { n: '一對一防守課', for: 'OUT', cond: weak('def'), gt: '搶球時機抓得剛剛好', bt: '老是被過，信心受挫', g: { def: 2 }, b: { def: -2 } },
  { n: '影像分析會議', for: '*', cond: weak('vis'), gt: '看穿了對手的跑位習慣', bt: '想太多，場上反而慢半拍', g: { vis: 2, pos: 2 }, b: { vis: -2 } },
  { n: '傳球節奏訓練', for: 'OUT', cond: weak('pas'), gt: '出球速度快了半拍', bt: '傳球失誤率上升', g: { pas: 2 }, b: { pas: -2 } },
  { n: '一對一過人課', for: 'OUT', cond: weak('dri'), gt: '假動作騙倒了整條防線', bt: '太貪帶球，被隊友抱怨', g: { dri: 2 }, b: { dri: -2 } },
  { n: '角球攻防加練', for: 'OUT', cond: weak('hea'), gt: '搶到第一點的能力變強了', bt: '空中對抗吃了幾次虧', g: { hea: 2 }, b: { hea: -2 } },
  { n: '撲救反應訓練', for: 'GK', cond: weak('ref'), gt: '近距離反應快得嚇人', bt: '重心亂了，撲救變慢', g: { ref: 2 }, b: { ref: -2 } },
  { n: '出擊時機判斷', for: 'GK', cond: weak('aer'), gt: '出擊時機抓得精準', bt: '幾次貿然出擊被吊門', g: { aer: 2 }, b: { pos: -2 } },
  { n: '門將長傳訓練', for: 'GK', cond: weak('lng'), gt: '一腳長傳直接找到前鋒', bt: '長傳老是找不到人，控球權一直送出去', g: { lng: 2 }, b: { lng: -2 } },
  { n: '後場短傳配合', for: 'GK', cond: weak('sht'), gt: '被逼搶時你總能把球傳出去', bt: '禁區內出球被斷，丟了球', g: { sht: 2 }, b: { sht: -2 } },
  { n: '教練戰術改造', for: '*', cond: newAtClub(1), gt: '新體系裡你變成關鍵齒輪', bt: '完全不適應新的要求', g: { rand: 2 }, b: { rand: -2 } },
  ...
  { n: '更衣室內鬨', for: 'PRO', cond: s => role(s) <= 2 || s.career.fanRep < 50, gt: '你出面把話講開了', bt: '被貼上刺頭標籤', g: { vis: 1, sta: 1 }, b: { rand: -2 } },
  { n: '青訓小將挑戰你的位置', for: 'PRO', cond: s => s.player.age >= 26, gt: '你用表現讓他坐回板凳', bt: '訓練賽被完全壓制', g: { rand: 2 }, b: { rand: -2 } },
  { n: '飲食與睡眠管理', for: '*', cond: weak('sta'), gt: '體脂下降，回復速度變快', bt: '作息亂掉，整季昏昏沉沉', g: { sta: 2 }, b: { sta: -2, inj: 4 } },
  { n: '客場長征與時差', for: 'PRO', cond: s => LV[s.club.lv].g >= 38, gt: '調整得宜，落地就進入狀況', bt: '整整兩週沒睡好', g: { sta: 1 }, b: { sta: -2, inj: 5 } },
  { n: '老將的一句話', for: '*', cond: s => s.player.age <= 25, gt: '一句話點醒夢中人', bt: '學了不適合自己的東西', g: { rand: 2 }, b: { rand: -2 } },
  { n: '季中低潮', for: '*', cond: s => lastRating(s) > 0 && lastRating(s) < 6.6, gt: '靠自我調整走了出來', bt: '低潮拖了一個月', g: { vis: 1, sta: 1 }, b: { fin: -2, sta: -1 } },
  { n: '語言與新環境', for: 'ABROAD', cond: newAtClub(2), gt: '你開始用當地語言在場上喊人', bt: '聽不懂戰術會議，訓練賽站錯位', g: { vis: 2, sta: 1 }, b: { vis: -2, sta: -1 } },
  { n: '思鄉', for: 'ABROAD', cond: newAtClub(3), gt: '把想家的力氣全放到球場上', bt: '整季心不在焉', g: { rand: 2 }, b: { rand: -2, inj: 4 } },
  { n: '當地媒體的檢視', for: 'ABROAD', cond: newAtClub(2), gt: '一場好球讓報紙改口叫你自己人', bt: '被寫成「便宜的外籍球員」', g: { fin: 1, dri: 1 }, b: { rand: -2 } },
  ...
  { n: '賽後把球衣丟給看台的小孩', for: 'PRO', cond: played(),
  ...
  { n: '訓練基地外等你的球迷', acts: ['揮手後上車', '停下來簽幾張', '簽到最後一個人為止'], for: 'PRO', cond: s => s.career.fanRep >= 45,
  ...
  { n: '遲到罰款', for: 'PRO', cond: s => s.player.age <= 24,
  ...
  { n: '隊內薪資曝光', for: 'PRO', cond: s => LV[s.club.lv].tier >= 4,
  ...
  { n: '慶祝脫球衣吃第二張黃', acts: ['克制住不脫', '照自己的方式慶祝', '脫了再說'], for: 'PRO', cond: played(),
  ...
  { n: '點球大戰主動舉手', acts: ['排在第五順位', '排中間順位', '第一個站出來'], for: 'PRO', cond: played(), weight: 0.6,
  ...
  { n: '爭議紅牌後炮轟裁判', acts: ['忍住不回應', '賽後訪問點到為止', '社群發文開炮'], for: 'PRO', cond: played(),
    gt: '球迷站在你這邊，罰款你自己付', bt: '協會加重處分',
    g: { fanRep: 12 }, b: {},
    fx: (s, win, api) => { if (win) api.fine(0.02); else api.suspend(4); } },
  { n: '門將上去頂角球', acts: ['留在自家禁區', '壓到中線接應', '直接衝進對方禁區'], for: 'GK', cond: played(), once: true, weight: 0.3,
    gt: '門將進球。整個球場瘋了。', bt: '你還在對方禁區，對面已經射空門了',
    g: { fanRep: 30 }, b: { fanRep: -10 },
    fx: (s, win) => { if (win) s.career.honors.push(`${s.career.year} 門將破門`); } },
  { n: '單場零封＋撲點', acts: ['冷靜受訪', '和後防線一起慶祝', '對著看台振臂大吼'], for: 'GK', cond: played(0.5),
  ...
  { n: '世界波遠射', acts: ['穩穩做球給隊友', '看情況起腳', '三十五碼直接轟'], for: 'PRO', cond: played(),
  ...
  /* ---------- 第七類：命運與機會（觸發率低、影響大） ---------- */
  { n: '巴西足球精靈的指點', for: 'PRO', once: true, weight: 0.1699,
  ...
  { n: '德國中場大師的私人課', for: 'PRO', once: true, weight: 0.1699,
  ...
  { n: '阿根廷球王的邀請', for: 'PRO', once: true, weight: 0.1485,
  ...
  { n: '日本三球王的邊路突破', for: 'PRO', once: true, weight: 0.1485,
  ...
  { n: '義大利鏈式防守的傳人', for: 'PRO', once: true, weight: 0.1485,
  ...
  { n: '巴西外星人的門前嗅覺', for: 'PRO', once: true, weight: 0.135,
    cond: s => LV[s.club.lv].region === 'SAM' || s.player.nation === 'BR',
  ...
  { n: '英國追風少年的加速課', for: 'PRO', once: true, weight: 0.1485,
  ...
  { n: '英國圓月彎刀的傳中課', for: 'PRO', once: true, weight: 0.1485,
    cond: s => LV[s.club.lv].region === 'EUR' || s.player.nation === 'GB',
  ...
  { n: '埃及法老的內切', for: 'PRO', once: true, weight: 0.1485,
  ...
  { n: '象牙海岸魔獸的對抗課', for: 'PRO', once: true, weight: 0.1485,
    cond: s => LV[s.club.lv].region === 'AFR' || ['NG', 'CI'].includes(s.player.nation),
  ...
  { n: '荷蘭冰王子的冷靜', for: 'PRO', once: true, weight: 0.135,
  ...
  { n: '飛翔的荷蘭人的凌空', for: 'PRO', once: true, weight: 0.135,
    cond: s => LV[s.club.lv].region === 'EUR' || s.player.nation === 'NL',
  ...
  { n: '台灣魔獸的重訓菜單', for: 'PRO', once: true, weight: 0.1699,
  ...
  { n: '土耳其火槍兵的遠射', for: 'PRO', once: true, weight: 0.135,
  ...
  { n: '台灣隊長的更衣室課', for: 'PRO', once: true, weight: 0.1699,
    cond: s => s.player.nation === 'TW' || s.player.natlPick === 'TW',
  ...
  { n: '阿根廷風之子的節奏', for: 'PRO', once: true, weight: 0.135,
  ...
  { n: '韓國三肺的跑動課', for: 'PRO', once: true, weight: 0.1485,
  ...
  { n: '法國忍者龜的第一步', for: 'PRO', once: true, weight: 0.135,
    cond: s => LV[s.club.lv].region === 'EUR' || s.player.nation === 'FR',
  ...
  { n: '西班牙小白的第一腳觸球', for: 'PRO', once: true, weight: 0.135,
  ...
  { n: '葡萄牙山羊的自我要求', for: 'PRO', once: true, weight: 0.1215,
    cond: s => LV[s.club.lv].region === 'EUR' || s.player.nation === 'PT',
  ...
  { n: '簽證卡關', for: 'ABROAD', cond: newAtClub(1), weight: 0.7,
  ...
  { n: '慈善義賽邀請', for: 'PRO', cond: s => s.career.fanRep >= 50, weight: 1.0,
...
export const CONF = {
  ...
  agingInjuryFrom: 31,
  agingInjury: 2.5,
  // 每季抽幾張事件卡。3 → 2 是刻意的減量：一局生涯本來要抽 88 張，
  // 玩到後面每一年都長得一樣。張數減少的能力增減由 evScale 補回來，
  // 讓「少而重」取代「多而淡」。
  eventCards: 2,
  // 事件卡效果幅度的全域倍率。**只作用在能力增減**（見 applyEffects）：
  // 聲望與受傷率乘上去會把重傷率推到 68%，並推歪一整排掛在 fanRep 門檻上的機制。
  evScale: 1.5,
  // 同一張卡幾年內不再進抽卡池。冷卻會系統性提高稀有卡的相對命中率 ——
  // 調這個數字一定要連 gpot 卡的 weight 一起看（8 年時 gpot 沒調降，峰值 P95
  // 會從 87 漲到 89、五大登陸率飆到 22%）。
  eventCooldown: 8,
  aclCap: 120,
