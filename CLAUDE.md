...
- **平衡只調到一半**，實測見 README 表格。五大聯賽登陸率（7.3%）與各國分布已經在目標內，但「登上洲際頂級以上 ~88%」與「重傷率 ~86%」仍偏高，入手點是 `LV` 第 3–4 層的 `min` 與 `CONF.baseInjury`。改動時記得跑單國模式對表。
- 養成階段共 6 年（國中 3 + 高中／足球學校 3），訓練骰數在 `flow.js:diceCount` 依 `club.stage` 分段。動這裡等於動整條成長曲線，一定要重跑模擬。
