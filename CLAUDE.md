...
`SYNERGY` 是連帶成長表，實作在 `addAb(p, k, points, spill)` 裡：帶動的點數一樣走蓄力槽，
...
**能力尺度與 `LV.par` 是綁在一起的**：任何讓整體能力上移的改動（連帶成長、潛力、骰數），
都要同步檢查 `LV` 的 `par`/`min`、`callThreshold` 的上限、以及 `accrueLoad` 的 explosive 除數，
否則會分別造成「聯賽門檻形同虛設」「人人都是國腳」「韌帶量表爆得特別快」。
...
- **平衡只調到一半**，實測見 README 表格。五大聯賽登陸率（13.8%）、重傷率（52%）與各國分布都在目標內，但「登上洲際頂級以上 ~88%」仍偏高，入手點是 `LV` 第 3–4 層的 `min`。改動時記得跑單國模式對表。
