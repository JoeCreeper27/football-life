...
**抽卡節奏（三個互相牽動的旋鈕，改一個要看另外兩個）**：

| 旋鈕 | 現值 | 動了會怎樣 |
|---|---|---|
| `CONF.eventCards` | 2 | 一局抽卡數。曾經是 3，一局生涯抽 88 張，玩到後面每年都一樣 |
| `CONF.evScale` | 1.5 | 補回張數減半的能力增減。**只乘能力**，`fanRep` 與 `inj` 不吃 —— 乘上去重傷率會從 62% 漲到 68% |
| `CONF.eventCooldown` | 8 | 抽過的卡幾年內不再進池。**會系統性抬高稀有卡的命中率** |

冷卻與 `gpot` 卡是綁在一起的：冷卻把常見卡擠出池子，24 張傳奇指導卡（全部是 `once`）
的相對權重就會上升。CD 8 而 `gpot` 權重沒調的話，峰值 ovr P95 從 87 漲到 89、
五大登陸率飆到 22%。現在那批卡的 weight 已經對應 CD 8 調過（×0.75）。

`raisePot`（`gpot`）**不吃 `evScale`** —— 天賦上限是單向且永久的。

**訓練型事件卡一律掛 `weak('能力')`**（低於自己計分能力的平均才會出現）。
無條件的訓練卡曾經佔掉全部抽卡的四分之一，而且它們和訓練骰做的是同一件事，
只是敘事更淡。同理，靠出場才會發生的卡（世界波、脫球衣慶祝、點球大戰）掛 `played()`，
新環境的卡掛 `newAtClub(n)` —— 一張卡「有理由在這個時候出現」，重複感就會少很多。
