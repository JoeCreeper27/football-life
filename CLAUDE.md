...
能力分成 `PHYSICAL`（速度／體能／對抗）與其餘技術類：體能類 26 歲起成本 ×2、33 歲起 `isGrowable()`
擋掉。衰退走 `DECLINE` 表（小數累進，取年齡符合的最後一段）：技術類 38 歲才開始掉、每年 0.15–0.25 點，
同期速度是 2.1 點 —— **差一個量級才是重點，不是「不掉」**。掉幅再乘上體能修正（體能越好掉得越慢），
而體能本身也在掉，這個正回饋就是斷崖式衰退的來源。
